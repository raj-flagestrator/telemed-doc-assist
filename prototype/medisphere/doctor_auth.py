"""Prototype doctor OTP — uses practitioners registry + shared identity_otp table."""
...
from medisphere.practitioners import DOCTOR_ACCOUNTS

PROTOTYPE_OTP = "123456"


async def doctor_otp_request(email: str, tenant_id: str | None = None) -> dict:
    settings = get_settings()
    tenant_id = tenant_id or settings.default_tenant_id
    normalized = email.strip().lower()
    if normalized not in DOCTOR_ACCOUNTS:
        raise HTTPException(404, "Doctor account not found in prototype directory")
...
async def doctor_otp_verify(email: str, code: str, tenant_id: str | None = None) -> dict:
    settings = get_settings()
    tenant_id = tenant_id or settings.default_tenant_id
    normalized = email.strip().lower()
    account = DOCTOR_ACCOUNTS.get(normalized)
    if not account:
    ...
    token = sign_token(
        sub=f"doctor-{normalized}",
        tenant_id=tenant_id,
        roles=["doctor"],
        practitioner_id=account["practitionerId"],
